                ...
                if 'vid' in data and data['vid']:
                    for vid in data['vid']:
                        if vid in monitor.tracked_vehicles:
                            ...
                            if v_type == 'signal':
                                message = f"{license_plate}: Changing lanes without using the turn signal!"
                            elif v_type == 'crossing':
                                message = f"{license_plate}: Violation of crossing a solid line!"
                            ...
                            cv.putText(lane_frame, message, (20, y_position), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
...
class YOLOv8:
    ...
    def lanedete(self,frame, net, img_transforms, cfg, cls_num_per_lane, frame_h, frame_w, vehicle_index, fps):
        ...
        for code, lane in coords.items():
            ...
            if color_check and reject == False:
                ...
                if yellow_ratio > 0.09 or (yellow_ratio > white_ratio and yellow_ratio > red_ratio and yellow_ratio > 0.06):
                    ...
                    lane_type, duty = dotted_or_solid(yellow_mask, lane_mask, MIN_ABS_LEN) #如果type是unknow才用下面那個輪廓
                    if lane_type == "Unknown":
                        ...
                        if len(contours) >= 3:
                            cv.putText(draw_line_frame, "Dotted_Yellow_Line", (x + 20, y - 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                            self.lane[code]['type'] = "Dotted_Yellow_Line"
                        else:
                            cv.putText(draw_line_frame, "Solid_Yellow_Line", (x + 20, y - 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                            self.lane[code]['type'] = "Solid_Yellow_Line"
                    else:
                        cv.putText(draw_line_frame, f"{lane_type}_Yellow_Line", (x + 20, y - 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                        self.lane[code]['type'] = f"{lane_type}_Yellow_Line"
                    cv.polylines(draw_line_frame, [lane.reshape(-1, 1, 2)], isClosed=False, color=(0, 255, 255), thickness=5)
                    ...
                    if lane_type == "Unknown":
                        ...
                        if len(contours) >= 3:
                            cv.putText(draw_line_frame, "Dotted_White_Line", (x + 20, y - 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                            self.lane[code]['type'] = "Dotted_White_Line"
                        else:
                            cv.putText(draw_line_frame, "Solid_White_Line", (x + 20, y - 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)   
                            self.lane[code]['type'] = "Solid_White_Line"
                    else: 
                        cv.putText(draw_line_frame, f"{lane_type}_White_Line", (x + 20, y - 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                        self.lane[code]['type'] = f"{lane_type}_White_Line"
                    cv.polylines(draw_line_frame, [lane.reshape(-1, 1, 2)], isClosed=False, color=(255, 255, 255), thickness=5)

                elif red_ratio > 0.1 :
                    cv.putText(draw_line_frame, "Red_Line", (x + 20, y - 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)   
                    self.lane[code]['type'] = "Red_Line"
                    cv.polylines(draw_line_frame, [lane.reshape(-1, 1, 2)], isClosed=False, color=(0, 0, 255), thickness=5)
                else:
                    x, y = map(int, lane[0])
                    kernel_horizon = cv.getStructuringElement(cv.MORPH_RECT, (4, 12))
                    kernel_vertical = cv.getStructuringElement(cv.MORPH_RECT, (12, 4))
            ...
            else:#這一段else 因為發生prev_lane != None and check_color == False時會導致程式沒有把上次的線畫出來
                ...
                if self.lane[code]['type'] == "Dotted_White_Line" or self.lane[code]['type'] == "Solid_White_Line":
                    cv.putText(draw_line_frame, self.lane[code]['type'], (x + 20, y - 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                    cv.polylines(draw_line_frame, [lane.reshape(-1, 1, 2)], isClosed=False, color=(255, 255, 255), thickness=5)
                elif self.lane[code]['type'] == "Dotted_Yellow_Line" or self.lane[code]['type'] == "Solid_Yellow_Line":
                    cv.putText(draw_line_frame, self.lane[code]['type'], (x + 20, y - 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
                    cv.polylines(draw_line_frame, [lane.reshape(-1, 1, 2)], isClosed=False, color=(255, 255, 255), thickness=5)
                elif self.lane[code]['type'] == "Red_Line":
                    cv.putText(draw_line_frame, self.lane[code]['type'], (x + 20, y - 40), cv.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
                    cv.polylines(draw_line_frame, [lane.reshape(-1, 1, 2)], isClosed=False, color=(0, 0, 255), thickness=5)
        ...
        for idx in vehicle_index:
            ...
            x, y, w, h = bbox
            if idx == 1:#記得改回來
                cv.putText(draw_line_frame, f"{self.tracked_vehicles[idx]['lane_changing']}", (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                if self.tracked_vehicles[idx]['direction_change'] is not None:
                    cv.putText(draw_line_frame, f"{self.tracked_vehicles[idx]['direction_change']}", (x, y - 30), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
...
#---------------方向燈
    ...
    def draw_info(self, frame, vehicle_id, violation):
        ...
        x, y, w, h = bbox
        # 繪製車輛邊界框
        if violation == True and vehicle_id == 1:#記得改回來
            cv.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)
            if self.tracked_vehicles[vehicle_id]['cx'] is not None:
                cv.circle(frame, (self.tracked_vehicles[vehicle_id]['cx'], y+h), radius=4, color=(0, 255, 0), thickness=-1)
            cv.putText(frame, f"ID: {vehicle_id}", (x, y - 80), cv.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            if self.tracked_vehicles[vehicle_id]['license_plate']:
                cv.putText(frame, f"Plate: {self.tracked_vehicles[vehicle_id]['license_plate']}", (x, y - 60), cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

            color_map = {"OFF": (0, 0, 255), "2OFF": (0, 0, 255),"FLASHING": (0, 255, 0), "ON": (255, 255, 0)}
            left_color = color_map[self.tracked_vehicles[vehicle_id]['left_signal']]
            cv.putText(frame, f"L: {self.tracked_vehicles[vehicle_id]['left_signal']}", (x - 40, y + 15), cv.FONT_HERSHEY_SIMPLEX, 0.5, left_color, 1)
            # left_color = color_map["2OFF"]
            # cv.putText(frame, f"L: 2OFF", (x - 40, y + 15), cv.FONT_HERSHEY_SIMPLEX, 0.5, left_color, 1)
            #right_color = color_map["2OFF"]
            #cv.putText(frame, f"R: 2OFF", (x + w - 20, y + 15), cv.FONT_HERSHEY_SIMPLEX, 0.5, right_color, 1)
            right_color = color_map[self.tracked_vehicles[vehicle_id]['right_signal']]
            cv.putText(frame, f"R: {self.tracked_vehicles[vehicle_id]['right_signal']}", (x + w - 20, y + 15), cv.FONT_HERSHEY_SIMPLEX, 0.5, right_color, 1)
        elif violation == False and vehicle_id == 1:
